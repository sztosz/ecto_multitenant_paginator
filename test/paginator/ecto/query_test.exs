defmodule EctoPaginator.Paginator.Ecto.QueryTest do
  ...
  defp create_posts do
    ...
    Enum.each(1..2, fn i ->
      %Comment{body: "Body #{i}", post_id: unpublished_post.id} |> Repo.insert!()
    end)

    Enum.map(1..6, fn i ->
      %Post{title: "Title #{i}", body: "Body #{i}", published: true} |> Repo.insert!()
    end)
  end

  defp create_key_values do
    Enum.map(1..10, fn i ->
      %KeyValue{key: "key_#{i}", value: rem(i, 2) |> to_string} |> Repo.insert!()
    end)
  end
  ...
  describe "paginate" do
    ...
    test "uses defaults from the repo" do
      posts = create_posts()
      page = Post |> Post.published() |> Repo.paginate()
      assert page.page_size == 5
      assert page.page_number == 1
    ...
    test "it handles preloads" do
      create_posts()
      page = Post |> Post.published() |> preload(:comments) |> Repo.paginate()
      assert page.page_size == 5
      assert page.page_number == 1
      assert page.total_pages == 2
    end
    ...
    test "it handles offsets" do
      create_posts()
      page = Post |> Post.unpublished() |> Repo.paginate(options: [offset: 1])
      assert page.entries |> length == 0
      assert page.page_number == 1
      assert page.total_pages == 1
      page = Post |> Post.published() |> Repo.paginate(options: [offset: 2])
    ...
    test "can be provided the current page and page size as a params map" do
      posts = create_posts()
      page = Post |> Post.published() |> Repo.paginate(%{"page" => "2", "page_size" => "3"})
    ...
    test "can be provided the caller as options" do
      create_posts()
      parent = self()
      task = Task.async(fn -> Repo.paginate(Post, caller: parent) end)
    ...
    test "can be provided the caller as a map" do
      create_posts()
      parent = self()
      task = Task.async(fn -> Post |> Repo.paginate(%{"caller" => parent}) end)
      page = Task.await(task)
      assert page.page_size == 5
      assert page.page_number == 1
      assert page.total_entries == 7
    ...
    test "will respect the max_page_size configuration" do
      create_posts()
      page = Post |> Post.published() |> Repo.paginate(%{"page" => "1", "page_size" => "20"})
      assert page.page_size == 10
    end

    test "will respect total_entries passed to paginate" do
      create_posts()
      page = Post |> Post.published() |> Repo.paginate(options: [total_entries: 130])
      assert page.total_entries == 130
    end

    test "will use total_pages if page_numer is too large" do
      posts = create_posts()
      page = Post |> Post.published() |> Repo.paginate(page: 3)
      assert page.page_number == 2
    ...
    test "allows overflow page numbers if option is specified" do
      ...
      page =
        Post
        |> Post.published()
        |> Repo.paginate(page: 3, options: [allow_overflow_page_number: true])
    ...
    test "can be used on a table with any primary key" do
      create_key_values()
      page = KeyValue |> KeyValue.zero() |> Repo.paginate(page_size: 2)
    ...
    test "can be used with a group by clause on field other than id" do
      create_posts()
      page = Post |> group_by([p], p.body) |> select([p], p.body) |> Repo.paginate()
      assert page.total_entries == 7
    end
    ...
    test "pagination plays nice with distinct on in the query" do
      create_posts()
      page = Post |> distinct([p], asc: p.title, asc: p.inserted_at) |> Repo.paginate()
      assert page.page_size == 5
      assert page.page_number == 1
      assert page.total_entries == 7
      assert page.total_pages == 2
    ...
    test "pagination plays nice with absolute distinct in the query" do
      create_posts()
      page = Post |> distinct(true) |> Repo.paginate()
      assert page.page_size == 5
      assert page.page_number == 1
      assert page.total_entries == 7
      assert page.total_pages == 2
    ...
    test "pagination plays nice with a singular distinct in the query" do
      create_posts()
      page = Post |> distinct(:id) |> Repo.paginate()
      assert page.page_size == 5
      assert page.page_number == 1
      assert page.total_entries == 7
      assert page.total_pages == 2
